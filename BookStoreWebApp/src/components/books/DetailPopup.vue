<template>
  <v-container>
        ...
        <v-card-text>
          ...
          <v-row>
            <v-col cols="12">
              <p>Ship via</p>
              <v-divider></v-divider>
              <v-btn-toggle v-model="serviceTypeSelected" group class="text-center">
                <v-btn
    ...
    <ServicesConfirmation ref="servicesConfirmation" />
  </v-container>
</template>
...
import ServicesConfirmation from "../servicesConfirmations/ServicesConfirmation"
export default {
  components: {ServicesConfirmation},
  data() {
    return {
      detailDialog: false,
      item: {
      ...
      deliverComponent: ServiceTypes.MOTOBIKE
    };
  },
  computed: {
    ...mapGetters(["packages"])
    ...
    async confirm() {
      let deliveryInfor = {
        ...
        book: {
          ...
          thumbnail: this.item.volumeInfo.imageLinks
            ? this.item.volumeInfo.imageLinks.smallThumbnail
            : null,
          authors: this.item.volumeInfo.authors
            ? this.item.volumeInfo.authors.join(", ")
            : null,
          publishedDate: this.item.volumeInfo.publishedDate
        }
      };
      var confirmation = await this.buyBook(deliveryInfor);
      this.$refs.servicesConfirmation.open(confirmation.data);
      this.detailDialog = false;
    },
